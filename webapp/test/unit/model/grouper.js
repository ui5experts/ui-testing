sap.ui.define([
  ...
  function createResourceModel() {
    return new ResourceModel({
      bundleUrl: [$.sap.getModulePath("ui5experts.ui-testing"), "i18n/i18n.properties"].join("/")
    });
  }
  ...
  QUnit.test("Should group a price lesser equal 1000", function (assert) {
    // Arrange
    ...
    assert.strictEqual(oGrouperReturn.key, "LE1000", "The key is as expected for a low value");
  ...
  QUnit.test("Should group the price", function (assert) {
    // Arrange
    var oContextObject = createContextObject(5555.5),
      oGrouperReturn;
    ...
    // Assert
    oGrouperReturn = fnGroup(oContextObject);
    assert.strictEqual(oGrouperReturn.key, "GT1000", "The key is as expected for a high value");
