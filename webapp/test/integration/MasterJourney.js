sap.ui.define([
    ...
    opaTest("Should see the master list with all entries", function (Given, When, Then) {
      ...
      // Assertions
      Then.onTheMasterPage.iShouldSeeTheList().and.theListShouldHaveAllEntries().and.theHeaderShouldDisplayAllEntries().and.theListShouldContainOnlyFormattedUnitNumbers();
    });
    ...
    opaTest("Entering something that cannot be found into search field and pressing 'search' should display the list's 'not found' message", function (Given, When, Then) {
      ...
      // Assertions
      Then.onTheMasterPage.iShouldSeeTheNoDataTextForNoSearchResults().and.theListHeaderDisplaysZeroHits();
    });
    ...
    opaTest("MasterList Filtering on EstimatedCost/Value less than 100", function (Given, When, Then) {

      // Action
      When.onTheMasterPage.iOpenViewSettingsDialog().and.iSelectListItemInViewSettingsDialog("<EstimatedCost/Value>").and.iSelectListItemInViewSettingsDialog("<100 <EstimatedCost/Currency>").and.iPressOKInViewSelectionDialog();

      // Assertion
      Then.onTheMasterPage.theMasterListShouldBeFilteredOnUnitNumberValueLessThanTheGroupBoundary();
    });

    opaTest("MasterList Filtering on EstimatedCost/Value more than 100", function (Given, When, Then) {
      // Action
      When.onTheMasterPage.iOpenViewSettingsDialog().and.iSelectListItemInViewSettingsDialog(">100 <EstimatedCost/Currency>").and.iPressOKInViewSelectionDialog();
    ...
    opaTest("MasterList remove filter should display all items", function (Given, When, Then) {
      // Action
      When.onTheMasterPage.iOpenViewSettingsDialog().and.iPressResetInViewSelectionDialog().and.iPressOKInViewSelectionDialog();
    ...
    opaTest("Remove grouping from MasterList delivers initial list", function (Given, When, Then) {
      ...
      // Assertion
      Then.onTheMasterPage.theListShouldNotContainGroupHeaders().and.theListShouldHaveAllEntries();
    });
    ...
    opaTest("Grouping the master list and filtering it by the object identifier should deliver the initial list", function (Given, When, Then) {
      // Action
      When.onTheMasterPage.iGroupTheList().and.iSortTheListOnName();

      // Assertion
      Then.onTheMasterPage.theListShouldNotContainGroupHeaders().and.iTeardownMyAppFrame();
    });
